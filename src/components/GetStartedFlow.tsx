...
const GetStartedFlow: React.FC<GetStartedFlowProps> = ({ isOpen, onClose, onNavigate }) => {
  ...
  // Desktop only: lock page scrolling so nothing behind can be interacted with or revealed.
  useEffect(() => {
  ...
  const handleSectionClick = (sectionId: string) => {
    onClose();
    // Ensure overlay closes before scrolling.
    setTimeout(() => {
  ...
  const desktopOverlay = (
    <motion.div
      key="get-started-desktop-overlay"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
    ...
    >
      ...
      <motion.div
        ...
        onClick={onClose}
      >
        <div className="absolute inset-0 bg-gradient-to-br from-[hsl(var(--overlay-from))] via-[hsl(var(--overlay-via))] to-[hsl(var(--overlay-to))]" />
        <div className="absolute inset-0 bg-gradient-to-tr from-[hsl(var(--overlay-from-2)/0.55)] via-[hsl(var(--overlay-via-2)/0.35)] to-[hsl(var(--overlay-to-2)/0.55)]" />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-[hsl(var(--overlay-glow-1)/0.25)] via-transparent to-transparent" />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_left,_var(--tw-gradient-stops))] from-[hsl(var(--overlay-glow-2)/0.22)] via-transparent to-transparent" />
      </motion.div>
            ...
            {sections.map((section, index) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Shine effect */}
                  <div
                    className="absolute inset-0 bg-gradient-to-tr from-white/0 via-white/30 to-white/0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 translate-x-[-100%] group-hover:translate-x-[100%]"
                  ...
                  <div className="relative z-10">
                    ...
                    <h4 className="font-bold text-white text-lg mb-2">{section.title}</h4>

                    <p className="text-white/80 text-sm leading-relaxed mb-4">{section.description}</p>
  ...
  return (
    <AnimatePresence>
      {isOpen && isDesktop && typeof document !== "undefined" && createPortal(desktopOverlay, document.body)}
                ...
                {sections.map((section, index) => {
                  ...
                  return (
                    ...
                    >
                      {/* Card Background */}
                      <div
                        className={`absolute inset-0 bg-gradient-to-br ${section.gradient} opacity-80 group-hover:opacity-100 transition-opacity duration-300`}
                      />
                      ...
                      <div className="relative z-10">
                        ...
                        <h4 className="font-bold text-white text-sm mb-1 line-clamp-1">{section.title}</h4>

                        <p className="text-white/80 text-xs line-clamp-2 leading-relaxed">{section.description}</p>
