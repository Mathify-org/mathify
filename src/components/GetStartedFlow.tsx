...
const GetStartedFlow: React.FC<GetStartedFlowProps> = ({ isOpen, onClose, onNavigate }) => {
  ...
  // Desktop only: prevent the underlying page from scrolling while the overlay is open.
  React.useEffect(() => {
      ...
      {isOpen && (
        <>
          ...
          {!isMobile &&
            createPortal(
              <div
                className="hidden md:flex fixed inset-0 z-[9999] items-center justify-center"
                role="dialog"
                aria-modal="true"
              >
